...
Mouse.KeyDown:Connect(function(Key)
    if Key:lower() == getgenv().Target.Keybind:lower() then
        if Shared.Forbidden.PointAssist.Enabled then  -- Updated to new structure
            if IsTargeting then
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        if BodyPart then
            ...
            if Shared.Forbidden.PointAssist.Align then  -- Update this to the correct field in the new structure
                local humanoid = TargetPlayer.Character:FindFirstChildOfClass("Humanoid")
                if humanoid then
                    local moveDirection = humanoid.MoveDirection
                    predictedPosition = BodyPart.Position + (moveDirection * Shared.Forbidden.PointAssist.VelocityCompensation)  -- Updated to new structure
                end
            else
                local targetVelocity = TargetPlayer.Character.HumanoidRootPart.Velocity
                predictedPosition = BodyPart.Position + (targetVelocity * Shared.Forbidden.PointAssist.VelocityCompensation)  -- Updated to new structure
            end

            if predictedPosition then
                local DesiredCFrame = CFrame.new(Camera.CFrame.Position, predictedPosition)

                if Shared.Forbidden.PointAssist.StabilizationEnabled then  -- Updated to new structure
                    Camera.CFrame = Camera.CFrame:Lerp(DesiredCFrame, Shared.Forbidden.PointAssist.Stabilization)  -- Updated to new structure
                else
